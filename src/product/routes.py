...
from src.product.dtos import ProductListResponse, ProductSchema, ProductResponse
...
@product_routes.get("/all_product",response_model=ProductListResponse,status_code=status.HTTP_200_OK)
def get_all_product(page:int=Query(1,ge=1),
                    limit:int=Query(10,le=50),
                    db:Session=Depends(get_db)):
    return controller.get_product(db,page,limit)

@product_routes.get("/one_product/{product_id}",response_model=ProductResponse,status_code=status.HTTP_200_OK)
